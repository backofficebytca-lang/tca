 ...
 * The most chosen plan carries a white badge, like the reference's "Popular".
 */
export function FormulaCard({
  ...
  return (
    <article className="lift group flex h-full flex-col rounded-[18px] bg-mist p-3">
      ...
      <div className="flex flex-1 flex-col px-3 pb-3 pt-6 md:px-5">
        <p className="t-small text-gray">{card.label}</p>
        ...
        <div className="mt-6 flex flex-wrap items-end gap-x-2">
          ...
          {card.unit && <span className="t-small mb-1 text-gray">{card.unit}</span>}
        </div>
        <p className="t-small mt-2 text-gray">{card.priceNote}</p>

        <dl className="t-small mt-6 flex flex-col gap-1.5 border-t border-ink/10 pt-5">
          <div className="flex gap-2">
            <dt className="text-gray">Pour</dt>
            <dd>{card.pour}</dd>
          </div>
          <div className="flex gap-2">
            <dt className="text-gray">Volume</dt>
            <dd>{card.volume}</dd>
          </div>
        </dl>

        <p className="t-small mt-6 text-gray">{card.inclusHeading}</p>
